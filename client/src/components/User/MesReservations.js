				...
				{mesReservations &&
					mesReservations.map((resa, i) => (
						<div className={styles.reservationItem}>
							{(new Date(resa.dateDebut).getTime() - new Date().getTime()) / 86400000 > 3 && <HighlightOffIcon onClick={() => onDelete(mesReservations[i])} />} {new Date(resa.dateDebut).toLocaleDateString()} - {new Date(resa.dateFin).toLocaleDateString()} - {resa.etablissement} -{" "}
							{resa.suite.nom}
						</div>
					))}
			</div>
